
	<div class="container-fluid">
    <div class="col-sm-3 sidenav" style="background-color: #eeeeee">
    	<div class="profile-sidebar">
			<div class="profile-userpic">
				<img src="public/source/images/slide/logo.png" class="img-responsive" alt="">
			</div>
			<div class="profile-usertitle">
				<div class="profile-usertitle-name">Admin</div>
				<div class="profile-usertitle-status"><span class="indicator label-success"></span>Online</div>
			</div>
			<div class="clear"></div>
		</div>
      <form role="search">
			<div class="form-group">
				<input type="text" class="form-control" placeholder="Search">
			</div>
		</form>
     <ul class="nav menu">
			<li><a data-toggle="collapse" href="#sub-item-1">
				<em class="fa fa-user-o">&nbsp;</em> User <span data-toggle="collapse" href="#sub-item-1" class="icon pull-right"><em class="fa fa-plus"></em></span>
				</a>
				<ul class="children collapse" id="sub-item-1">
					<li><a class="" href="admin/user/index">
						&nbsp;</span> List
					</a></li>
					<li><a class="" href="admin/user/add">
						&nbsp;</span> Add
					</a></li>
					
				</ul>
			</li>
			<li><a data-toggle="collapse" href="#sub-item-2">
				<em class="fa fa-navicon">&nbsp;</em> Loại sản phẩm <span data-toggle="collapse" href="#sub-item-2" class="icon pull-right"><em class="fa fa-plus"></em></span>
				</a>
				<ul class="children collapse" id="sub-item-2">
					<li><a class="" href="admin/productType/index">
						&nbsp;</span> List
					</a></li>
					<li><a class="" href="admin/productType/add">
						&nbsp;</span> Add
					</a></li>
					
				</ul>
			</li>
			<li><a data-toggle="collapse" href="#sub-item-3">
				<em class="fa fa-navicon">&nbsp;</em> Sản phẩm <span data-toggle="collapse" href="#sub-item-3" class="icon pull-right"><em class="fa fa-plus"></em></span>
				</a>
				<ul class="children collapse" id="sub-item-3">
					<li><a class="" href="admin/product/index">
						&nbsp;</span> List
					</a></li>
					<li><a class="" href="admin/product/add">
						&nbsp;</span> Add
					</a></li>
						
				</ul>
			</li>
			<li><a data-toggle="collapse" href="#sub-item-4">
				<em class="fa fa-navicon">&nbsp;</em> Tin tức <span data-toggle="collapse" href="#sub-item-4" class="icon pull-right"><em class="fa fa-plus"></em></span>
				</a>
				<ul class="children collapse" id="sub-item-4">
					<li><a class="" href="admin/news/index">
						&nbsp;</span> List
					</a></li>
					<li><a class="" href="admin/news/add">
						&nbsp;</span> Add
					</a></li>
						
				</ul>
			</li>
				<li><a data-toggle="collapse" href="#sub-item-5">
				<em class="fa fa-navicon">&nbsp;</em> Slide <span data-toggle="collapse" href="#sub-item-5" class="icon pull-right"><em class="fa fa-plus"></em></span>
				</a>
				<ul class="children collapse" id="sub-item-5">
					<li><a class="" href="admin/slide/index">
						&nbsp;</span> List
					</a></li>
					<li><a class="" href="admin/slide/add">
						&nbsp;</span> Add
					</a></li>
						
				</ul>
			</li>
			
		</ul><br>
     
    </div>

    